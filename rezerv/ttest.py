...
def z1_3():
    ...
    with open(File1, 'r') as f1, open(File2, 'r') as f2, open(TemporaryFile3, 'w') as f4:
        os.system("mv %s %s" % (File1, 'names.tmp'))
        # переименовываем временный файл
        os.system("mv %s %s" % (File2, 'numbers.tmp'))
        os.system("mv %s %s" % ('names.tmp', File1))
        os.system("mv %s %s" % ('numbers.tmp', File2))  # пер
...
def z3z4_2(File1, File2):
    ...
    z1()
    global jek
    jek = input(f'{jir}Введите поряд.номер: ')
    if jek in list1:
...
def z4_3(File1, File2, FindThis1, FindThis2, new1, new2, error1, error2):
    TemporaryFile1 = File1 + '.tmp'
    TemporaryFile2 = File2 + '.tmp'   # создаём файл
    os.system("touch %s" % TemporaryFile1)
    os.system("touch %s" % TemporaryFile2)    # временный файл
    ...
    path1 = os.path.join(os.path.abspath(os.path.dirname(__file__)), File1)
    path2 = os.path.join(os.path.abspath(os.path.dirname(__file__)), File2)
    os.remove(path1)
    os.remove(path2)  # удаляем основной файл
    os.system("mv %s %s" % (TemporaryFile1, File1))
    # переименовываем временный файл
    os.system("mv %s %s" % (TemporaryFile2, File2))
    os.system(ff)
